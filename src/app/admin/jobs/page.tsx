...
export default function AdminJobsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const result = await adminService.getJobs(page, search || undefined, showDeleted);
      setJobs(result.data);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-5 py-3 font-medium text-gray-500">고용형태</th>
                <th className="text-left px-5 py-3 font-medium text-gray-500">지역</th>
                <th className="text-left px-5 py-3 font-medium text-gray-500">등록일</th>
              ...
              {loading ? (
                ...
                jobs.map((job) => (
                  <tr key={job.id} className={job.deleted_at ? 'bg-red-50/50 opacity-60' : 'hover:bg-gray-50'}>
                    ...
                    <td className="px-5 py-3 text-gray-600">{job.author?.company_name || job.author?.contact_name || '-'}</td>
                    <td className="px-5 py-3 text-gray-600">{getBusinessTypeLabel(job.business_type)}</td>
                    <td className="px-5 py-3 text-gray-600">{getEmploymentTypeLabel(job.employment_type)}</td>
                    <td className="px-5 py-3 text-gray-600">{getRegionLabel(job.region)}</td>
