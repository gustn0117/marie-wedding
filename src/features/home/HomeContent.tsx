  ...
  return (
    <div className="bg-background pb-8">
      <section className="bg-white border-b border-gray-200">
        <div className="max-w-[1440px] mx-auto px-3 sm:px-5 lg:px-6 xl:px-8 py-7">
          <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
            <div className="min-w-0">
              <div className="mb-4 flex flex-wrap items-center gap-2 text-sm font-semibold text-gray-600">
                <span className="rounded border border-primary px-2 py-0.5 text-primary">웨딩업계 채용 플랫폼</span>
                <span>채용, 업체 섭외, 커뮤니티를 한 번에</span>
              </div>

              <form onSubmit={handleSearch} className="flex min-h-[58px] overflow-hidden rounded border-2 border-primary bg-white">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
                ...
                {searchTags.map((tag) => (
                  <Link key={tag} href={`${ROUTES.JOBS}?search=${encodeURIComponent(tag)}`} className="rounded border border-gray-200 px-3 py-1 text-gray-600 hover:border-primary hover:text-primary transition-colors">
                    {tag}
                  </Link>
                ))}
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                <SummaryCard label="오늘 올라온 공고" value="128" href={ROUTES.JOBS} />
                <SummaryCard label="등록 업체" value="2,430" href={ROUTES.DIRECTORY} />
                <SummaryCard label="커뮤니티 글" value="8,920" href={ROUTES.COMMUNITY} />
              </div>
            </div>
            ...
            <aside className="saramin-section p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="rounded border border-gray-300 px-1.5 py-0.5 text-[11px] font-bold text-gray-700">무료</span>
              </div>
      ...
      <div className="max-w-[1440px] mx-auto px-3 sm:px-5 lg:px-6 xl:px-8 py-5 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5">
              {BUSINESS_TYPES.map((type) => (
                <Link
                  ...
                  className="group flex min-h-[82px] flex-col justify-center border-b border-r border-gray-100 px-4 py-3 hover:bg-gray-50 transition-colors"
                >
                  <span className="mb-2 flex h-8 w-8 items-center justify-center rounded border border-gray-200 bg-white text-sm font-bold text-gray-700 group-hover:border-primary group-hover:text-primary transition-colors">
                    {type.label.charAt(0)}
                  </span>
                  <span className="text-sm font-semibold text-gray-800 group-hover:text-primary">{type.label}</span>
                </Link>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-1.5 p-4">
              {REGIONS.slice(0, 15).map((region) => (
                <Link
                  ...
                  className="rounded border border-gray-200 px-2 py-2 text-center text-sm font-semibold text-gray-700 hover:border-primary hover:text-primary transition-colors"
                >
                  {region.label}
                </Link>
              ))}
        ...
        <section className="saramin-section">
          <div className="saramin-section-title flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span>추천 채용정보</span>
              <span className="rounded border border-gray-300 px-1.5 py-0.5 text-[10px] font-bold text-gray-600">광고</span>
            </div>
            <Link href={ROUTES.JOBS} className="text-xs font-semibold text-gray-500 hover:text-primary">더보기</Link>
          </div>
          <div className="divide-y divide-gray-100">
            {featuredJobs.map((job) => (
              <Link key={`${job.company}-${job.title}`} href={ROUTES.JOBS} className="group grid gap-3 px-4 py-4 hover:bg-gray-50 transition-colors md:grid-cols-[190px_1fr_150px] md:items-center">
                <div className="min-w-0">
                  <p className="truncate text-sm font-bold text-gray-900">{job.company}</p>
                  <p className="mt-1 text-xs text-gray-500">{job.region}</p>
                </div>
                <div className="min-w-0">
                  <h3 className="truncate text-[16px] font-bold text-gray-900 group-hover:text-primary transition-colors">{job.title}</h3>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    <span className="rounded border border-gray-200 px-1.5 py-0.5 text-[11px] font-semibold text-gray-700">{job.type}</span>
                    <span className="rounded border border-gray-200 px-1.5 py-0.5 text-[11px] font-semibold text-gray-700">{job.pay}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between gap-2 md:justify-end">
                  <span className="text-xs font-semibold text-gray-500">상시채용</span>
                  <span className="rounded border border-gray-300 px-3 py-1.5 text-xs font-bold text-gray-700 group-hover:border-primary group-hover:text-primary transition-colors">상세보기</span>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="saramin-section">
            ...
            <div className="grid gap-0 sm:grid-cols-2">
              {partnerCompanies.map((company) => (
                <Link key={company.name} href={ROUTES.DIRECTORY} className="group flex gap-3 border-b border-r border-gray-100 p-4 hover:bg-gray-50 transition-colors">
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded border border-gray-200 bg-white text-lg font-bold text-gray-700 group-hover:border-primary group-hover:text-primary transition-colors">
                    {company.name.charAt(0)}
                  </span>
                  <span className="min-w-0">
                    <span className="block truncate text-sm font-bold text-gray-900 group-hover:text-primary transition-colors">{company.name}</span>
          ...
          <div className="saramin-section">
            ...
            <div className="divide-y divide-gray-100">
              {posts.length > 0 ? posts.map((post) => (
                <Link key={post.id} href={ROUTES.COMMUNITY_DETAIL(post.id)} className="block px-4 py-3 hover:bg-gray-50 transition-colors group">
                  ...
                  <p className="line-clamp-1 text-sm font-bold text-gray-900 group-hover:text-primary transition-colors">{post.title}</p>
...
function SummaryCard({ label, value, href }: { label: string; value: string; href: string }) {
  return (
    <Link href={href} className="rounded border border-gray-200 bg-white px-4 py-3 hover:border-primary transition-colors group">
      <span className="block text-xs font-semibold text-gray-500">{label}</span>
      <span className="mt-1 block text-2xl font-bold text-gray-900 group-hover:text-primary transition-colors">{value}</span>
    </Link>
  );
}
