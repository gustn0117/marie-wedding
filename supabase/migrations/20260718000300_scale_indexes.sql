...
-- 라이브에는 CREATE INDEX CONCURRENTLY 로 무잠금 생성 완료. 이 파일은 bootstrap/release
-- 러너(트랜잭션 내 실행 가능)와 재실행 안전을 위해 IF NOT EXISTS 일반 생성으로 둔다
-- (기존 인덱스가 있으면 skip, 신규/빈 DB 에선 즉시 생성).

-- 1) /jobs 기본 목록 정렬(is_promoted DESC, created_at DESC). 기존엔 이 조합 인덱스가
--    없어 활성공고 풀스캔+Sort. 부분조건은 목록 쿼리 필터와 일치시켜 LIMIT push-down.
CREATE INDEX IF NOT EXISTS idx_jobs_list_promoted_created
...
CREATE INDEX IF NOT EXISTS idx_snapshots_snapshot_gin
...
CREATE INDEX IF NOT EXISTS idx_profiles_contact_name_trgm
